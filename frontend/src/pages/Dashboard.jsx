      ...
      <main className="p-8 flex-1 ml-20 md:ml-30 mr-6 transition-all duration-300">
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className={`p-4 rounded-lg shadow ${darkMode ? "bg-gray-800" : "bg-white"}`}>
            ...
            <p className="text-2xl font-bold">{studyStats?.totalTime ? `${Math.floor(studyStats.totalTime / 60)}h` : "0h"}</p>
          </div>
          <div className={`p-4 rounded-lg shadow ${darkMode ? "bg-gray-800" : "bg-white"}`}>
            <p className="text-sm text-gray-400">Storage Used</p>
            <p className="text-2xl font-bold">{libraryStats.totalSize ? `${(libraryStats.totalSize / (1024 * 1024)).toFixed(1)} MB` : "0 MB"}</p>
          </div>
        </section>
